...
%
% contiene le funzioni ritardo della 3.2.5 e NON della 3.2.6
% sistemato il salvataggio itermedio dei valori delle porte
                ...
                if block.Dwork(5).Data>0
                   ...
                   if state==1
                       ...
                       for i=1: block.Dwork(5).Data
                           ...
                           if block.Dwork(6).Data(i)==0 %CONTINUOUS
                                ...
                                add_query=strcat('(''', string(cisia.id_run),''',''' , num2str(cisia.major_time),''',''', block.DialogPrm(1).Data,''',''', block.Dwork(i+cisia.offset).name,''',', '''CONTINUOUS''',',''', num2str(block.ContStates.data(j)),''')');
                                %execute(cisia.conn,query)
                           elseif block.Dwork(6).Data(i)==1 % NUMERIC
                                ...
                                add_query=strcat('(''', string(cisia.id_run),''',''' , num2str(cisia.major_time),''',''', block.DialogPrm(1).Data,''',''', block.Dwork(i+cisia.offset).name,''',', '''NUMERIC''',',''', num2str(block.Dwork(i+cisia.offset).Data),''')');
                                %execute(cisia.conn,query)
                           elseif block.Dwork(6).Data(i)==3 % NUMERIC_ARRAY
                                ...
                                add_query=strcat('(''', string(cisia.id_run),''',''' , num2str(cisia.major_time),''',''', block.DialogPrm(1).Data,''',''', block.Dwork(i+cisia.offset).name,''',', '''NUMERIC_ARRAY''',',''', num2str(block.Dwork(i+cisia.offset).Data'),''')');
                                %execute(cisia.conn,query)
                           elseif block.Dwork(6).Data(i)==2 % STRING
                                ...
                                add_query=strcat('(''', string(cisia.id_run),''',''' , num2str(cisia.major_time),''',''', block.DialogPrm(1).Data,''',''', block.Dwork(i+cisia.offset).name,''',', '''STRING''',',''', strtrim(string(char(block.Dwork(i+cisia.offset).Data)')),''')');
                                %execute(cisia.conn,query)
                           else
                   ...
                   if cisia.save_ports==1 & state==1
                        ...
                        for i=1:num_outputs
                            ...
                            data_entity_port.sim_time(i)=cisia.major_time;   %block.CurrentTime;
...
if strcmp(num2str(strfind(block.DialogPrm(1).Data,'SAVE')),'1')  % SAVE ENTITY
    ...
    if state==1
        ...
        query=strcat('INSERT INTO run_output (id_run, sim_time, name_entity, name_var, val_type, val_status) VALUES (''', string(cisia.id_run),''',''' , num2str(cisia.major_time),''',''', block.DialogPrm(1).Data,''',''', strtrim(string(char(block.Dwork(1).Data)')),''',', '''RESOURCE''',',''', data,''')');
        if ~cisia.skipsaving
            execute(cisia.conn,query)
        end
